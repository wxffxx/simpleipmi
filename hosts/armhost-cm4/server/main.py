...
Generic CM4-compatible Baseboard Management Controller

Features:
  - MJPEG video stream from USB capture card
  - HID keyboard/mouse via ESP32-S3 serial bridge (or legacy ConfigFS)
  - GPIO power control (on/off/reset) & 12V status
  - Web dashboard + KVM remote desktop
  - REST API + WebSocket
  - UART auto-discovery for ESP32-S3
...
from modules.esp32_flasher import ESP32Flasher
...
esp32_flasher = ESP32Flasher(config.get("esp32_flasher", {}))
esp32_flasher.set_hid_manager(hid_manager)
...
async def lifespan(app: FastAPI):
    ...
    # Setup ESP32-S3 flasher
    await esp32_flasher.setup()
    if esp32_flasher.get_status()["available"]:
        log_buffer.add("ESP32-S3 OTA flasher ready")
    else:
        log_buffer.add("ESP32-S3 OTA flasher disabled (GPIO not configured)", "WARNING")
...
app = FastAPI(
    title="SI BMC Server",
    description="Generic CM4-compatible Baseboard Management Controller",
    version="2.0.0",
    lifespan=lifespan,
)
...
# ── UART Port Management API ───────────────────────────────────
@app.get("/api/hid/ports")
async def hid_list_ports():
    """List all detected UART ports and ESP32-S3 connection status."""
    return await hid_manager.get_ports()


class PortSelectRequest(BaseModel):
    port: str

@app.post("/api/hid/ports/select")
async def hid_select_port(req: PortSelectRequest, _auth=Depends(check_auth)):
    """Select and switch to a specific UART port for ESP32-S3 HID."""
    result = await hid_manager.select_port(req.port)
    if "error" in result:
        raise HTTPException(status_code=400, detail=result["error"])
    log_buffer.add(f"HID port switched to {req.port}")
    return result


@app.post("/api/hid/ports/probe")
async def hid_probe_ports(_auth=Depends(check_auth)):
    """Re-scan all UART ports for ESP32-S3."""
    result = await hid_manager.probe_ports()
    log_buffer.add("UART port probe completed")
    return result


# ── ESP32-S3 OTA Flash API ──────────────────────────────────────
from fastapi import File, UploadFile

@app.get("/api/esp32/status")
async def esp32_flash_status():
    """Get ESP32-S3 flasher status and progress."""
    return esp32_flasher.get_status()


@app.post("/api/esp32/flash")
async def esp32_flash_firmware(
    firmware: UploadFile = File(...),
    _auth=Depends(check_auth)
):
    """
    Upload and flash firmware to ESP32-S3.
    Accepts a .bin file. The ESP32-S3 will be reset into download mode,
    flashed, and rebooted automatically.
    """
    if not firmware.filename.endswith(".bin"):
        raise HTTPException(status_code=400, detail="Only .bin firmware files accepted")

    # Save uploaded file
    firmware_dir = config.get("esp32_flasher", {}).get("firmware_dir", "/opt/si-bmc/firmware")
    os.makedirs(firmware_dir, exist_ok=True)
    firmware_path = os.path.join(firmware_dir, "latest.bin")

    content = await firmware.read()
    if len(content) > 4 * 1024 * 1024:
        raise HTTPException(status_code=400, detail="Firmware too large (max 4MB)")

    with open(firmware_path, "wb") as f:
        f.write(content)

    log_buffer.add(f"ESP32-S3 firmware uploaded: {firmware.filename} ({len(content)} bytes)", "WARNING")

    # Flash
    result = await esp32_flasher.flash_firmware(firmware_path)

    if result["ok"]:
        log_buffer.add("ESP32-S3 firmware flash completed successfully")
        return result
    else:
        log_buffer.add(f"ESP32-S3 firmware flash failed: {result.get('error')}", "ERROR")
        raise HTTPException(status_code=500, detail=result.get("error", "Flash failed"))


@app.post("/api/esp32/reset")
async def esp32_reset(_auth=Depends(check_auth)):
    """Reset ESP32-S3 (EN pulse) without flashing."""
    result = await esp32_flasher.reset_esp32()
    if result["ok"]:
        log_buffer.add("ESP32-S3 reset", "WARNING")
        return result
